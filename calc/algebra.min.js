    ...
    1: [function(e, t, r) {
      ...
      u.prototype.solveFor = function(e) {
        ...
        if (this._isLinear() || this._variableCanBeIsolated(e)) {
          ...
          var r = new n;
          var u = new n;
          for (var f = 0; f < this.rhs.terms.length; f++) {
            var l = this.rhs.terms[f];
            if (l.canBeCombinedWith(t)) {
              r = r.subtract(l)
            } else {
              u = u.add(l)
            }
          }
          for (var f = 0; f < this.lhs.terms.length; f++) {
            var l = this.lhs.terms[f];
            if (l.canBeCombinedWith(t)) {
              r = r.add(l)
            } else {
              u = u.subtract(l)
          ...
          u = u.subtract(this.lhs.constant());
          u = u.add(this.rhs.constant());
          if (r.terms.length === 0) {
            if (r.constant().equalTo(u.constant())) {
              return new o(1, 1)
            } else {
          ...
          if (r.terms.length === 0) {
            ...
            var c = r._quadraticCoefficients();
            var p = c.a;
            var h = c.b;
            var v = c.c;
            var m = h.pow(2).subtract(p.multiply(v).multiply(4));
            if (m.valueOf() >= 0) {
              if (m.valueOf() === 0) {
                return [h.multiply(-1).divide(p.multiply(2)).reduce()]
              } else {
                var d;
                if (m._squareRootIsRational()) {
                  d = m.pow(.5);
                  var y = h.multiply(-1).subtract(d).divide(p.multiply(2));
                  var g = h.multiply(-1).add(d).divide(p.multiply(2));
                  return [y.reduce(), g.reduce()]
                } else {
                  d = Math.sqrt(m.valueOf());
                  p = p.valueOf();
                  h = h.valueOf();
                  var y = (-h - d) / (2 * p);
                  var g = (-h + d) / (2 * p);
            ...
            var v = c.c;
            var b = c.d;
            var w = p.multiply(h).multiply(v).multiply(b).multiply(18);
            w = w.subtract(h.pow(3).multiply(b).multiply(4));
            w = w.add(h.pow(2).multiply(v.pow(2)));
            w = w.subtract(p.multiply(v.pow(3)).multiply(4));
            w = w.subtract(p.pow(2).multiply(b.pow(2)).multiply(27));
            var _ = h.pow(2).subtract(p.multiply(v).multiply(3));
            if (w.valueOf() === 0) {
              if (_.valueOf() === 0) {
                var y = h.multiply(-1).divide(p.multiply(3));
                return [y.reduce()]
              } else {
                var y = p.multiply(h).multiply(v).multiply(4);
                y = y.subtract(p.pow(2).multiply(b).multiply(9));
                y = y.subtract(h.pow(3));
                y = y.divide(p.multiply(_));
                var g = p.multiply(b).multiply(9).subtract(h.multiply(v)).divide(_.multiply(2));
                return [y.reduce(), g.reduce()]
              }
            } else {
              var T = new n("t").subtract(h.divide(p.multiply(3)));
              var x = {};
              x[e] = T;
              var E = r.eval(x);
              var S = E._cubicCoefficients();
              var p = S.a.valueOf();
              var h = S.b.valueOf();
              var v = S.c.valueOf();
              var b = S.d.valueOf();
              if (w.valueOf() < 0) {
                var I = -h / 3 * p;
                var M = Math.pow(I, 3) + (h * v - 3 * p * b) / (6 * p ^ 2);
                var O = v / 3 * p;
                var R = Math.sqrt(Math.pow(M, 2) + Math.pow(O - Math.pow(I, 2), 3));
                var A = Math.cbrt(M + R) + Math.cbrt(M - R) + I;
                A = a(A) ? new o(A, 1) : A;
                var x = {};
                x[e] = Math.round(A);
                A = r.eval(x).toString() === "0" ? new o(Math.round(A), 1) : A;
                return [A]
              } else {
                var M = Math.sqrt(-3 * p * v / (9 * Math.pow(p, 2)));
                var D = 2 * p * Math.pow(M, 3);
                var k = 1 / 3 * Math.acos(-b / D);
                var P = 2 * M * Math.cos(k);
                var V = 2 * M * Math.cos(2 * Math.PI / 3 - k);
                var C = 2 * M * Math.cos(2 * Math.PI / 3 + k);
                var F = P + T.constant().valueOf();
                var q = V + T.constant().valueOf();
                var N = C + T.constant().valueOf();
                F = a(F) ? new o(F, 1) : F;
                q = a(q) ? new o(q, 1) : q;
                N = a(N) ? new o(N, 1) : N;
                var L = {};
                ...
                W[e] = Math.round(q);
                U[e] = Math.round(N);
                F = r.eval(L).toString() === "0" ? new o(Math.round(F), 1) : F;
                q = r.eval(W).toString() === "0" ? new o(Math.round(q), 1) : q;
                N = r.eval(U).toString() === "0" ? new o(Math.round(N), 1) : N;
                return [N, q, F]
              }
            }
          }
